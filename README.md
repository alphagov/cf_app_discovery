...
To test the service broker during development, you will need to change the following in the `manifest.yml`:

- BUCKET_NAME   should be set to `gds-prometheus-targets-staging` so that the targets 
- SERVICE_ID    needs a unique guid
- SERVICE_NAME  needs to be unique
- PLAN_ID       needs a unique guid
- service broker name
- target updater name

New guid can be generated by running `uuidgen | awk '{print tolower($0)}'`
